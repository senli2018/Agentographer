...
def load_mdh(path):
    ...
    # 获取所有切片
    slices = [image[:, :, i] for i in range(image.GetSize()[2])]
...
def load_scan(path):
    ...
    for s in os.listdir(path):
        try:
            ...
            slice = pydicom.read_file(full_path)
            slices.append(slice)
        except (pydicom.errors.InvalidDicomError, StopIteration) as e:
